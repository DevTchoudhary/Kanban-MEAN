...
var _BreakpointObserver = class _BreakpointObserver {
  constructor(_mediaMatcher, _zone) {
    ...
    this._destroySubject = new import_rxjs5.Subject();
  }
...
var Typeahead = class {
  constructor(initialItems, config) {
    this._letterKeyStream = new import_rxjs6.Subject();
    this._items = [];
    this._selectedItemIndex = -1;
    this._pressedLetters = [];
    this._selectedItem = new import_rxjs6.Subject();
...
var TreeKeyManager = class {
  ...
  constructor(items, config) {
    ...
    this._items = [];
    this._typeaheadSubscription = import_rxjs6.Subscription.EMPTY;
    this._hasInitialFocused = false;
    this.change = new import_rxjs6.Subject();
...
var _InputModalityDetector = class _InputModalityDetector {
  ...
  constructor(_platform, ngZone, document2, options) {
    ...
    this._modality = new import_rxjs6.BehaviorSubject(null);
    this._lastTouchMs = 0;
...
var _FocusMonitor = class _FocusMonitor {
  constructor(_ngZone, _platform, _inputModalityDetector, document2, options) {
    ...
    this._stopInputModalityDetector = new import_rxjs6.Subject();
...
var DragRef = class {
  ...
  constructor(element, _config, _document, _ngZone, _viewportRuler, _dragDropRegistry) {
    ...
    this._moveEvents = new import_rxjs7.Subject();
    this._pointerMoveSubscription = import_rxjs7.Subscription.EMPTY;
    this._pointerUpSubscription = import_rxjs7.Subscription.EMPTY;
    this._scrollSubscription = import_rxjs7.Subscription.EMPTY;
    ...
    this.scale = 1;
    this._disabled = false;
    this.beforeStarted = new import_rxjs7.Subject();
    this.started = new import_rxjs7.Subject();
    this.released = new import_rxjs7.Subject();
    this.ended = new import_rxjs7.Subject();
    this.entered = new import_rxjs7.Subject();
    this.exited = new import_rxjs7.Subject();
    this.dropped = new import_rxjs7.Subject();
...
var DropListRef = class {
  constructor(element, _dragDropRegistry, _document, _ngZone, _viewportRuler) {
    ...
    this.exited = new import_rxjs7.Subject();
    this.dropped = new import_rxjs7.Subject();
    this.sorted = new import_rxjs7.Subject();
    this.receivingStarted = new import_rxjs7.Subject();
    this.receivingStopped = new import_rxjs7.Subject();
    this._isDragging = false;
    this._draggables = [];
    this._siblings = [];
    this._activeSiblings = /* @__PURE__ */ new Set();
    this._viewportScrollSubscription = import_rxjs7.Subscription.EMPTY;
    this._verticalScrollDirection = AutoScrollVerticalDirection.NONE;
    this._horizontalScrollDirection = AutoScrollHorizontalDirection.NONE;
    this._stopScrollTimers = new import_rxjs7.Subject();
    this._cachedShadowRoot = null;
    this._scrollableElements = [];
    this._direction = "ltr";
    this._startScrollInterval = () => {
      this._stopScrolling();
      (0, import_rxjs7.interval)(0, import_rxjs7.animationFrameScheduler).pipe((0, import_operators5.takeUntil)(this._stopScrollTimers)).subscribe(() => {
...
var _DragDropRegistry = class _DragDropRegistry {
  constructor(_ngZone, _document) {
    ...
    this.pointerMove = new import_rxjs7.Subject();
    this.pointerUp = new import_rxjs7.Subject();
    this.scroll = new import_rxjs7.Subject();
  ...
  scrolled(shadowRoot) {
    ...
    if (shadowRoot && shadowRoot !== this._document) {
      streams.push(new import_rxjs7.Observable((observer) => {
        return this._ngZone.runOutsideAngular(() => {
    ...
    return (0, import_rxjs7.merge)(...streams);
  }
  ngOnDestroy() {
...
var _CdkDrag = class _CdkDrag {
  ...
  constructor(element, dropContainer, _document, _ngZone, _viewContainerRef, config, _dir, dragDrop, _changeDetectorRef, _selfHandle, _parentDrag) {
    ...
    this._parentDrag = _parentDrag;
    this._destroyed = new import_rxjs7.Subject();
    this._handles = new import_rxjs7.BehaviorSubject([]);
    this.scale = 1;
    ...
    this.moved = new import_rxjs7.Observable((observer) => {
      const subscription = this._dragRef.moved.pipe((0, import_operators5.map)((movedEvent) => ({
        source: this,
    ...
    if (dropContainer) {
      ...
      dropContainer._dropListRef.beforeStarted.pipe((0, import_operators5.takeUntil)(this._destroyed)).subscribe(() => {
  ...
  _syncInputs(ref) {
    ...
    ref.beforeStarted.pipe((0, import_operators5.take)(1)).subscribe(() => {
      if (this._parentDrag) {
  ...
  _setupHandlesListener() {
    this._handles.pipe(
      ...
      (0, import_operators5.tap)((handles) => {
      ...
      (0, import_operators5.switchMap)((handles) => {
        return (0, import_rxjs7.merge)(...handles.map((item) => item._stateChanges.pipe((0, import_operators5.startWith)(item))));
      }),
      (0, import_operators5.takeUntil)(this._destroyed)
...
var _CdkDropList = class _CdkDropList {
  ...
  _setupInputSyncSubscription(ref) {
    if (this._dir) {
      this._dir.change.pipe((0, import_operators5.startWith)(this._dir.value), (0, import_operators5.takeUntil)(this._destroyed)).subscribe((value) => ref.withDirection(value));
    }
    ref.beforeStarted.subscribe(() => {
  ...
  _handleEvents(ref) {
    ...
    (0, import_rxjs7.merge)(ref.receivingStarted, ref.receivingStopped).subscribe(() => this._changeDetectorRef.markForCheck());
  }
  /** Assigns the default input values based on a provided config object. */
  _assignDefaults(config) {
    const {
